...
describe('Client', () => {
    it('should connect to a server', async () => {
        let client = new Client('ws://localhost:8888')
        await delay(60)
    ...
    it('should send messages', async() => {
        let client = new Client('ws://localhost:8888')
        await delay(60)
        let sendFunction = jest.spyOn(client.socket, 'send')
        client.send('test', 'test')
        let message = JSON.stringify(['test', 'test'])
    ...
    it('should broadcast messages', async() => {
        ...
        let message = JSON.stringify(['test', 'test', true])
        expect(sendFunction).toHaveBeenCalledWith(message)
        client.close()
    })
    ...
    it('should queue messages when not connected', async() => {
        let client = new Client('ws://localhost:8888', { autoConnect: false })
        client.send('test', 'test')
        expect(client.queue.length).toBe(1)
    })
    it('should not exceed queue size when queuing messages', async() => {
        let client = new Client('ws://localhost:8888', { maxQueueSize: 2, autoConnect: false })
        client.send('test', 'test')
    ...
    it('should reconnect automatically', async() => {
        ...
        let connectFunction = jest.spyOn(client, 'connect')
        // Force close
        client.socket.close()
    ...
    it('should retry initial connect automatically', async() => {
        let client = new Client('invalid-url')
        // Set 10ms reconnect interval
        client.reconnectInterval = .01
        await delay(60)
        // Spy on connect function
